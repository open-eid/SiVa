...
import eu.europa.esig.dss.validation.policy.SignatureFormatConstraint;
...
public class ValidationContextInitialisation {
	...
	private boolean checkSignatureFormat(final Conclusion conclusion) {
		SignatureFormatConstraint constraint = constraintData.getSignatureFormatConstraint();
		if (constraint == null) {
			return true;
		}

		constraint.create(subProcessNode, MessageTag.BBB_VCI_ISFC);
