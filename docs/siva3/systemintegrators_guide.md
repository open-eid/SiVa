...
```bash
./siva-parent/siva-webapp/target/siva-webapp-3.2.0.jar
```

**Second we need to start X-road validation webapp**

```bash
./validation-services-parent/xroad-validation-service/target/xroad-validation-service-3.2.0.jar
```
...
```bash
./siva-parent/siva-sample-application/target/siva-sample-application-3.2.0.jar
```
...
Next we need to move `siva-webapp-3.2.0.jar` into newly created `/var/apps` directory and rename to
...
```bash
sudo mkdir /var/apps
sudo cp siva-parent/siva-webapp/target/executable/siva-webapp-3.2.0.jar /var/apps/siva-webapp.jar
```
...
```bash
cp siva-parent/siva-webapp/target/siva-webapp-3.2.0.war apache-tomcat-8.5.24/webapps
