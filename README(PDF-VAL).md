...
```bash
 ...
 mvn test
```
Verify that the service_url property in src/main/config/test.properties points to the correct running service path
Some of the test files are used to sign with test certificate (chain), so in order for all the tests to pass run the servlet container with development profile:
```bash
...
PDF Validator monitoring is a web service to check that the PDF validator service is running correctly. Run monitoring:
```bash
