...
public class ValidationProxy {
    ...
    public QualifiedReport validate(ProxyDocument proxyDocument) {
        ...
        if (proxyDocument.getDocumentType() != null && proxyDocument.getDocumentType() == DocumentType.XROAD) {
            ...
            report = getServiceForType(proxyDocument.getName()).validateDocument(createValidationDocument(proxyDocument));
        }
        statisticsService.publishValidationStatistic(System.nanoTime() - validationStartTime, report);
        return report;
    }

    private ValidationService getServiceForType(String fileName) {
        String validatorName = constructValidatorName(fileName);
        try {
    ...
    private static String constructValidatorName(String fileName) {
